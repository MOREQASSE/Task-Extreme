...
function createTaskElement(task) {
  ...
  li.innerHTML = `
    ...
    <div class="task-actions">
      ...
      <button class="delete-task" data-task-id="${task.id}" title="Delete task">
...
function getAllCategories() {
  // Gather all categories from the static form select (task-category)
  const select = document.getElementById('task-category') || document.getElementById('edit-task-category');
  if (!select) return [];
  const cats = [];
  for (let i = 0; i < select.options.length; i++) {
    const opt = select.options[i];
    if (opt.value && opt.value !== '') cats.push(opt.value);
  }
  return cats;
}

function applyTaskFilters(taskList) {
  const {
    query, priority, category, status
  } = searchFilterState;
  return taskList.filter(task => {
    // Search query (title, details, category, date)
    ...
    if (query) {
      ...
      matchesQuery = (
        ...
        (task.date && task.date.toLowerCase().includes(q))
      );
    }
    // Priority
    let matchesPriority = !priority || (task.priority && task.priority.toLowerCase() === priority);
    // Category
    let matchesCategory = !category || (task.category && task.category.toLowerCase() === category.toLowerCase());
    // Status
    let matchesStatus = true;
    if (status === 'completed') {
      matchesStatus = !!checked[task.id];
...
function populateCategoryFilter() {
  ...
  catSelect.innerHTML = '<option value="">All Categories</option>' +
    cats.map(cat => `<option value="${cat}">${cat.charAt(0).toUpperCase() + cat.slice(1)}</option>`).join('');
}
...
function setupSearchAndFilters() {
  ...
  searchInput.addEventListener('input', debounce(e => {
    searchFilterState.query = e.target.value.trim();
    renderTasks();
  }, 180));

  prioritySelect.addEventListener('change', e => {
    searchFilterState.priority = e.target.value;
    renderTasks();
  });
  categorySelect.addEventListener('change', e => {
    searchFilterState.category = e.target.value;
    renderTasks();
  });
  statusSelect.addEventListener('change', e => {
    searchFilterState.status = e.target.value;
    renderTasks();
  });
...
// ===== MODIFIED renderTasks TO APPLY FILTERS =====
const origRenderTasks = renderTasks;
renderTasks = function() {
  if (!tasksList || !tasksDayTitle) return;
  // Always repopulate categories for up-to-date filter menu
  populateCategoryFilter();
  let dayTasks = getTasksForDate(currentDate);
  // Apply search/filter
  dayTasks = applyTaskFilters(dayTasks);
  // Update title
  tasksDayTitle.textContent = formatDateForDisplay(currentDate);
  // Show/hide delete all button
  const deleteAllBtn = document.getElementById('delete-all-tasks-btn');
  ...
  tasksList.innerHTML = '';
  if (dayTasks.length === 0) {
    tasksList.innerHTML = `<li class="task-item empty-state premium-empty"><div class="task-content"><p>No tasks match your search or filters.</p><small>Try adjusting your search or clearing filters.</small></div></li>`;
  } else {
    dayTasks.forEach(task => {
      const taskElement = createTaskElement(task);
      tasksList.appendChild(taskElement);
